...
--历练笔记，通过任务ID获取相关战斗
local fightList = nil
local function GetFightListByQuestId(quest_id)
    if not fightList then
        fightList = {}
        DATABASE.ForEach("lilian_transition", function(row)
            fightList[row.quest_id] = fightList[row.quest_id] or {}
            table.insert(fightList[row.quest_id], row)
        end)
    end
    return fightList[quest_id]
end
...
return {
	...
	GetFightList = GetFightListByQuestId,
}
