...
local function Hero(id)
    ...
    return setmetatable({type=TYPE.HERO, id = id, cfg = cfg}, {__index = function(t, k)
        ...
        elseif k == "quality" or k== "role_stage" then--hero品质
            -- local hero = HeroModule.GetManager():Get(id);
            ...
            --     return 0;
            -- end
            --新的 hero 品质 不再跟 阶数挂钩，直接读配置role_stage
            if cfg and cfg.role_stage then
                return cfg.role_stage
            else
                return 0;
            end
...
local function Equipment(uuid, id)
    ...
    return setmetatable({type=TYPE.EQUIPMENT, uuid = uuid, id = id, cfg = cfg}, {__index=function(t,k)
...
local function Inscription(uuid, id)
    ...
    return setmetatable({type=TYPE.INSCRIPTION, uuid = uuid, id = id}, {__index=function(t,k)
        ...
        if k == "count" then
            return equip and 1 or 0;
        elseif k == "cfg" then
            return equipmentConfig.InscriptionCfgTab()[id] or {name = "未知铭文(" .. id .. ")", icon = "10000", quality = 0,  type = 0}
        elseif k == "sub_type" then
...
local function Suit(id)
    local cfg = HeroScroll.GetSuitCfg(id);  
    if not cfg then
        if not not_exists_id[id] and UnityEngine.Application.isEditor then
            -- print("item config not exist", id);
            not_exists_id[id] = true;
        end
       cfg = {name = "未知套装(" .. id .. ")", icon = "10000", quality = 0,  type = 0};
    end

    return setmetatable(cfg, {__index=function(t, k)
        if k == "type" then
            return TYPE.SUIT;
        end
    end});
