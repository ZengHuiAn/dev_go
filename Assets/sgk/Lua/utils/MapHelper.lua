...
local function opUnionShop()
    DialogStack.PushMapScene("ShopFrame", {index = 4})
end
...
local function quickToUse(itemType, itemId, effectName, effTime, showType, play_icon, play_text, btnName, isAuto, func)
    if itemType and itemId then
        if isAuto then
            local _item = SGK.ResourcesManager.Load("prefabs/effect/UI/"..effectName..".prefab")
...
local function EnterOthersManor(pid)
	...
	if playerModule.Get().level < openLevel.GetCfg(2001).open_lev  then
		showDlgError(nil,SGK.Localize:getInstance():getValue("guild_player_base"))
		return;
	end

	module.playerModule.Get(pid,function ( p_info )
		
		if p_info.level < openLevel.GetCfg(2001).open_lev then
			showDlgError(nil,SGK.Localize:getInstance():getValue("guild_player_base"))
			-- guild_player_base
			return
		end
...
local function OpenDrawCard()
    DialogStack.Push("DrawCard/newDrawCardFrame", data)
end
