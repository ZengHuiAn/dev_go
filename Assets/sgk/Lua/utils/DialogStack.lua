...
function DialogStack:PushPref(name, arg, parObj, add, func)
    ...
    if not string.find(name, "/") then
        prefabName = "base/" .. name;
    end

    self:LoadAsync("prefabs/"..prefabName .. ".prefab",function (prefab)
        ...
        local dialog;
        if name == "StoryFrame" then
            dialog = CS.UnityEngine.GameObject.Instantiate(prefab)
        else
            dialog = CS.UnityEngine.GameObject.Instantiate(prefab,parent.transform)
        end
        ...
        local funcs = loadfile(scriptFileName);
        if funcs then
            controller = ControllerProfiler(funcs(), scriptFileName);
        end
        if controller then
...
function DialogStack:deActive(top)
    ...
    if #self.stack == 0 then
        ...
        module.guideModule.PlayByType(1000, 0.2)
    end
end
...
local function SetInstance(instance)
    ...
    for i,v in ipairs(insertDialogList) do
        _instance:Push(v.name, v.arg, v.tag)
    end
    insertDialogList = {};
...
local thread = Thread.Create(function()
    while true do
        ...
        -- print("DialogStack", cmd, name, arg, parObj, func);
        if cmd == "PUSH" then
...
local function CheckInserStack()
    --[[
    for i,v in ipairs(insertDialogList) do
        ERROR_LOG("CheckInSertStack", v.name);
        GetInstance():Push(name, arg,parObj)
        -- Push()
    end
