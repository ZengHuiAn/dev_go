...
--个人信息面板统计资源
local totalItemConfig = nil
local function GetTotalItemConfig()
    if not totalItemConfig then
        totalItemConfig = LoadDatabaseWithKey("role_rewardinfo", "id");
    end
    return totalItemConfig
end
...
local function GetShowItemList(type)
    if showItemsList[type] == nil then
        ...
        for i,v in ipairs(list) do
            ...
            local _temp = setmetatable({type=utils.ItemHelper.TYPE.ITEM,effect=v.effect,effect_type=v.effect_type,group=v.group,text_color = v.text_color},{__index=cfg})
    ...
    if showItemsList[type] then
        table.sort(showItemsList[type],function(a,b)
            local owned_a = module.ItemModule.GetItemCount(a.id)>0
            local owned_b = module.ItemModule.GetItemCount(b.id)>0
            if owned_a~= owned_b then
                return owned_a
            end
            return a.id<b.id  
        end)
...
local function FilterQueryType(pid, types)
    local info = PlayerQueryTimeTab[pid]

    if not info then
        return types
    end

    local dead_line = os.time() - delayTime;
    
    local list = {}
    for _, t in ipairs(types) do
        if (info[t] or 0) < dead_line then
            table.insert(list, t);
        end
    end
...
local function RecordQueryTime(pid, ...)
    PlayerQueryTimeTab[pid] = PlayerQueryTimeTab[pid] or {}
    local now = os.time();
    for _, type in ipairs({...}) do
        PlayerQueryTimeTab[pid][type] = now;
    end
end
...
local function QueryPlayerAddData(pid,type,func,Reget)--Reget是否重新获取
    ...
    if not Reget then
        typeTab = FilterQueryType(pid, typeTab)
    end

    RecordQueryTime(pid, table.unpack(typeTab));
    ...
    if not playerAddData[pid] or Reget then
        return
    end
...
EventManager.getInstance():addListener("server_respond_17082", function(event, cmd, data)
    ...
    ERROR_LOG("server_respond_17082",sprinttb(data),data[3],PlayerAddDataSnFunction[data[1]]);
    local err = data[2];
    if err == 0 then
    ...
    if err == 0 then
        if data[3] and data[4]  then
            ...
            if PlayerAddDataSnData[sn] or PlayerAddDataSnData[sn]==false then
                ...
                DispatchEvent("PLAYER_ADDDATA_CHANGE_SUCCED", PlayerAddDataSnData[sn], data[4][1], _pid);--修改成功 
...
return {
    ...
    GetTotalItemCfg = GetTotalItemConfig,
