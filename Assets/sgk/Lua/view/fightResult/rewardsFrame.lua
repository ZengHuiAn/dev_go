...
function View:Update()
	...
	if v[1] ~= 90 and v[2] ~= 90000  then
		...
		if item.is_show ~= 0 then
			...
			-- local start=SGK.UIReference.Setup(obj)
			-- if start then
			-- 	start.transform:SetParent(self.reward_parent_transform, false);

			-- 	utils.IconFrameHelper.Create(start.IconFrame,{type=type,id=id,uuid=uuid,count=value, showDetail = true,onClickFunc=function ()
			-- 		local UIRootParent = UnityEngine.GameObject.FindWithTag("battle_root").gameObject
			-- 		if UIRootParent then
			-- 			local _root = SGK.UIReference.Setup(UIRootParent).Canvas.UIRoot
			-- 			DialogStack.PushPrefStact("ItemDetailFrame", {id = id,type = type,uuid =uuid},_root.gameObject)
			...
			-- 		end
			-- 	end})
			-- 	start.TopImage.gameObject:SetActive(showLeadReward and ShowDoubleRewardTipTab[id])
			-- end
		end
    end
end
